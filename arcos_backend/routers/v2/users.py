...
from ...davult.models import User as m_User
...
def get_all_users(db: Annotated[Session, Depends(get_db)]) -> list[int]:
    return [user.id for user in user_db.get_users(db) if not user.is_deleted and json.loads(user.properties)['acc']['enabled']]
